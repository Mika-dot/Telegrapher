...
OpenJsCad.Processor.prototype = {
  ...
  createElements: function() {
    ...
    var this_ = this;
    [['faces', 'поверхности', this.options.drawFaces],
     ['lines', 'линия', this.options.drawLines]].forEach(function(tup) {
    ...
    this.abortbutton.innerHTML = "Прервать";
    ...
    headerdiv.textContent = "Параметры:";
    ...
    parseParametersButton.innerHTML = "Обновлять";
  ...
  updateDownloadLink: function() {
    ...
    this.generateOutputFileButton.innerHTML = "Создать "+ext.toUpperCase();
  },
    ...
    var startTime = Date.now();
      ...
      this.worker = OpenJsCad.parseJsCadScriptASync(this.script, paramValues, this.options, function(err, obj) {
        ...
        else
        {
          ...
          that.statusspan.innerHTML = "Готов." + (that.options.verbose ?
              "  Отрендерено в " + elapsed + " миллисекунд" : "");
        }
        that.enableItems();
  ...
  downloadLinkTextForCurrentObject: function() {
    var ext = this.selectedFormatInfo().extension;
    return "Скачать "+ext.toUpperCase();
  },
